...
red135_boost_1f1 = snrs_cnd1_1f1_avg - snrs_cnd3_1f1_avg
red135_boost_1f2 = snrs_cnd2_1f2_avg - snrs_cnd4_1f2_avg
green45_boost_1f1 = snrs_cnd4_1f1_avg - snrs_cnd2_1f1_avg
green45_boost_1f2 = snrs_cnd3_1f2_avg - snrs_cnd1_1f2_avg

# average across all channels
red135_boost = np.mean((red135_boost_1f1 + red135_boost_1f2) / 2)
green45_boost = np.mean((green45_boost_1f1 + green45_boost_1f2) / 2)
...
red135_boost_occ = np.mean(((red135_boost_1f1 + red135_boost_1f2) / 2)[ind_occ])
green45_boost_occ = np.mean(((green45_boost_1f1 + green45_boost_1f2) / 2)[ind_occ])
# ----------------------------------------------------------------------------
...
im, _ = mne.viz.plot_topomap(red135_boost_1f1, epochs.info, axes=axes[0, 2],
                             vlim=(vmin, vmax), show=False)
axes[0, 2].set(title='red135 boost 1f1')
cp.add_snr_colorbar(fig, axes[0, 2], im)
# Topography map of red135 boost at 1f2:
im, _ = mne.viz.plot_topomap(red135_boost_1f2, epochs.info, axes=axes[1, 2],
                             vlim=(vmin, vmax), show=False)
axes[1, 2].set(title='red135 boost 1f2')
cp.add_snr_colorbar(fig, axes[1, 2], im)

# Topography map of green45 boost at 1f1:
im, _ = mne.viz.plot_topomap(green45_boost_1f1, epochs.info, axes=axes[2, 2],
                             vlim=(vmin, vmax), show=False)
axes[2, 2].set(title='green45 boost 1f1')
cp.add_snr_colorbar(fig, axes[2, 2], im)
# Topography map of green45 boost at 1f2:
im, _ = mne.viz.plot_topomap(green45_boost_1f2, epochs.info, axes=axes[3, 2],
                             vlim=(vmin, vmax), show=False)
axes[3, 2].set(title='green45 boost 1f2')
...
axs[0].bar([1, 2], [red135_boost, green45_boost], color='grey')
axs[0].set(title='All channels', xticks=[1, 2],
           xticklabels=['red135', 'green45'], ylabel='SNR improvement')
cp.trim_axes(axs[0])
cp.prep4ai()

axs[1].bar([1, 2], [red135_boost_occ, green45_boost_occ], color='grey')
axs[1].set(title='Occipital channels', xticks=[1, 2],
           xticklabels=['red135', 'green45'])
cp.trim_axes(axs[1])
cp.prep4ai()
