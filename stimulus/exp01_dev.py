...
change_frame_list1 = list(range(480, 540 + 1, 1))  # between 8-9 sec
change_frame_list2 = list(range(120, 300 + 1, 1))  # between 2-5 sec
...
for itrial in range(N_TRIALS):
    ...
    change_frame1 = random.choice(change_frame_list1)
    change_frame2 = random.choice(change_frame_list2)
    ...
    change_time1 = float("nan")
    change_time2 = float("nan")
    response_time1 = float("nan")
    response_time2 = float("nan")
    ...
    for iframe in range(TRIAL_DUR):
        ...
        if (iframe > change_frame1) & (
                iframe < change_frame1 + CHANGE_DUR):
            if change_image == 1:
                ...
                rel_image3.draw()
                rel_image1.draw()
        elif (iframe > change_frame2) & (
                iframe < change_frame2 + CHANGE_DUR):
            if change_image == 1:
                rel_image3.pos = (path1_x[iframe], path1_y[iframe])
                rel_image2.draw()
                rel_image3.draw()
        ...
        if iframe == change_frame1:
            change_time1 = timer.getTime()
        if iframe == change_frame2:
            change_time2 = timer.getTime()
        ...
        event.clearEvents()
        if change_frame1 < iframe < (change_frame1 + 60):
            if 'space' in response_key and not response1:
                # get response
                response_key = event.getKeys(keyList=['space'])
                response1 = 1
                response_time = timer.getTime()
                # measure reaction time in ms and end trial
                RT1 = round((response_time - change_time1) * 1000, 0)
                if math.isnan(RT1):
                    RT1 = -1
                    print(f"RT1: Early   ", end="")
                else:
                    print(f"RT1: {int(RT1):03d}ms   ", end="")
        if iframe > (change_frame1 + 60) and (response1 == 0):
            RT1 = float("nan")
            print(f"RT: None    ", end="")

        if 'space' in response_key and response < 2:
            response += 1
            if response == 1:
                response_time = timer.getTime()
                # measure reaction time in ms and end trial
            ...
            if response == 2:
                response_time = timer.getTime()
                # measure reaction time in ms and end trial
                RT2 = round((response_time - change_time2) * 1000, 0)
                if math.isnan(RT2):
                    print(f"RT2: Early   ", end="")
                else:
                    print(f"RT2: {int(RT2):03d}ms   ", end="")
    # ------------------ main period ends
    if response == 0:
        print(f"RT: None    ", end="")
    # evaluate the response
    resp_eval = sup.evaluate_response(cue_image, change_image, RT1, response)
